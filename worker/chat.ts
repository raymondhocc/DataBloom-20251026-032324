...
export class ChatHandler {
  ...
  private async handleStreamResponse(
    ...
    if (accumulatedToolCalls.length > 0) {
      ...
      onChunk(finalResponse); // Stream the final tool response
    ...
    return Promise.all(
      openAiToolCalls.map(async (tc) => {
        try {
          ...
          // Inject sessionId for chat workflow
          if (tc.function.name === 'trigger_n8n_chat_workflow') {
    ...
    return [
      {
        ...
        content: 'You are DataBloom AI, a helpful assistant that helps users interact with their data. You can answer questions, search the web, and query user-uploaded data through N8N workflows. Be concise and helpful.'
      },
